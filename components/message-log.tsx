...
export function MessageLog({ messages, onUpdate }: MessageLogProps) {
    ...
    <Card>
      <CardHeader className="flex flex-row items-start justify-between">
        <div>
          ...
          <CardDescription>Track email conversations with this influencer</CardDescription>
        </div>
        <Button onClick={() => setIsAddingMessage(true)}>
      ...
      <CardContent className="space-y-4">
        {messages.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground">No messages logged yet</div>
        ) : (
          <div className="space-y-4">
            {messages.map((message) => (
              <div
                key={message.id}
                className={`p-4 border rounded-md ${message.direction === "incoming" ? "bg-muted/30" : "bg-primary/5"}`}
              >
        ...
        <Dialog open={isAddingMessage} onOpenChange={setIsAddingMessage}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>Log a new email conversation</DialogDescription>
            </DialogHeader>
