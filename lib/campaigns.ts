...
export async function getAllCampaigns(userId: string): Promise<Campaign[]> {
  try {
    ...
    return result.map(row => ({
      ...
      startDate: row.start_date ? (row.start_date instanceof Date ? row.start_date.toISOString() : row.start_date) : undefined,
      endDate: row.end_date ? (row.end_date instanceof Date ? row.end_date.toISOString() : row.end_date) : undefined,
...
export async function getCampaignById(id: string): Promise<CampaignWithInfluencers | null> {
  try {
    ...
    return {
      ...
      startDate: campaign.start_date ? (campaign.start_date instanceof Date ? campaign.start_date.toISOString() : campaign.start_date) : undefined,
      endDate: campaign.end_date ? (campaign.end_date instanceof Date ? campaign.end_date.toISOString() : campaign.end_date) : undefined,
      budget: campaign.budget ? parseFloat(campaign.budget) : undefined,
      status: campaign.status,
      briefUrl: campaign.brief_url,
      notes: campaign.notes,
      createdAt: campaign.created_at ? (campaign.created_at instanceof Date ? campaign.created_at.toISOString() : campaign.created_at) : undefined,
      updatedAt: campaign.updated_at ? (campaign.updated_at instanceof Date ? campaign.updated_at.toISOString() : campaign.updated_at) : undefined,
      influencers,
    };
  } catch (error) {
